...
utils.raise_on_missing_env_vars(["BLENDERKIT_API_KEY", "ASSET_AUTHOR_ID"])
...
def run_some_workload(asset_data: dict[str, Any], api_key: str) -> None:
    """Bogus thread that does nothing.
...
def main() -> None:
    ...
    # search for assets created by specific user

    asset_owner = os.getenv("ASSET_AUTHOR_ID")
    if not asset_owner:
        raise ValueError("ASSET_AUTHOR_ID environment variable is not set.")

    params = {
        "author_id": f"{asset_owner}",
        # "author.fullName": "Blenderkit Community"
    }
    ...
    for a in assets:
        # Fixed-width columns: name 40 chars, type 15 chars, id full UUID
        logger.debug(
            " %-40.40s | %-15.15s | %s | %s",
            a.get("name", ""),
            a.get("assetType", ""),
