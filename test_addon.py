...
TODO: figure out, how to pass the success/error message outside to aggregating Github workflow,
    which will then comment on the asset.
"""
...
def test_addon(addon_data, api_key, binary_path: str) -> tuple[bool, dict]:
    ...
    addon_file_path = download.download_asset(
        addon_data, api_key=api_key, directory=tempfile.gettempdir(), filetype='zip_file')
    ...
    result_path = os.path.join(
        temp_folder, addon_data['assetBaseId'] + '_resdata.json')
...
if __name__ == '__main__':
    ...
    if len(addons) == 0:
        raise Exception("Addon not found in the database")  # noqa: TRY002

    # One result is expected, but for transparency..
    for i, asset in enumerate(addons):
        print(
            f"{i + 1}. {asset['assetType']}: {asset['name']} ({asset['url']})")

    # We just take 1st result
    test_ok, test_results = test_addon(
        addons[0], API_KEY, binary_path=BLENDER_PATH)
