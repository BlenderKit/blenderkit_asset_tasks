...
def _set_value_safer(target_socket: bpy.types.NodeSocket, value: Any) -> None:  # noqa: C901
    ...
    if target_socket is None:
        logger.warning("Target socket is None.")
        return
    if not hasattr(target_socket, "default_value"):
        logger.warning("Socket %r has no default_value.", getattr(target_socket, "name", target_socket))
        return
            ...
            if isinstance(coerced, (tuple, list)) and hasattr(dv, "__len__"):
                for i, c in enumerate(coerced):
                    try:
                        ...
                        logger.warning("Failed to set component %d on socket %r", i, getattr(target_socket, "name", target_socket))  # noqa: E501
            else:
                # last resort: try float cast
                target_socket.default_value = float(coerced)  # type: ignore[assignment]
        except Exception:
            logger.exception("Failed to set value on socket %r", getattr(target_socket, "name", target_socket))
...
def bkit_wireframe() -> bpy.types.Material:  # noqa: PLR0915
    ...
    # Use safer setter: pass sockets, not .default_value
    _set_value_safer(principled_bsdf.inputs[0], (0.0, 0.0, 0.0, 0.0))  # Base Color
    _set_value_safer(principled_bsdf.inputs[1], 0.0)  # Metallic
    _set_value_safer(principled_bsdf.inputs[2], 0.5)  # Roughness
    _set_value_safer(principled_bsdf.inputs[3], 1.5)  # IOR
    _set_value_safer(principled_bsdf.inputs[5], (0.0, 0.0, 0.0))  # Normal
    _set_value_safer(principled_bsdf.inputs[7], 0.0)  # Subsurface Weight
    _set_value_safer(principled_bsdf.inputs[8], (1.0, 0.2, 0.1))  # Subsurface Radius
    _set_value_safer(principled_bsdf.inputs[9], 0.05)  # Subsurface Scale
    _set_value_safer(principled_bsdf.inputs[11], 0.0)  # Subsurface Anisotropy
    _set_value_safer(principled_bsdf.inputs[12], 0.5)  # Specular IOR Level
    _set_value_safer(principled_bsdf.inputs[13], (1.0, 1.0, 1.0, 1.0))  # Specular Tint
    _set_value_safer(principled_bsdf.inputs[14], 0.0)  # Anisotropic
    _set_value_safer(principled_bsdf.inputs[15], 0.0)  # Anisotropic Rotation
    _set_value_safer(principled_bsdf.inputs[16], (0.0, 0.0, 0.0))  # Tangent
    _set_value_safer(principled_bsdf.inputs[17], 0.0)  # Transmission Weight
    _set_value_safer(principled_bsdf.inputs[18], 0.0)  # Coat Weight
    _set_value_safer(principled_bsdf.inputs[19], 0.03)  # Coat Roughness
    _set_value_safer(principled_bsdf.inputs[20], 1.5)  # Coat IOR
    _set_value_safer(principled_bsdf.inputs[21], (1.0, 1.0, 1.0, 1.0))  # Coat Tint
    _set_value_safer(principled_bsdf.inputs[22], (0.0, 0.0, 0.0))  # Coat Normal
    _set_value_safer(principled_bsdf.inputs[23], 0.0)  # Sheen Weight
    _set_value_safer(principled_bsdf.inputs[24], 0.5)  # Sheen Roughness
    _set_value_safer(principled_bsdf.inputs[25], (1.0, 1.0, 1.0, 1.0))  # Sheen Tint
    _set_value_safer(principled_bsdf.inputs[27], 1.0)  # Emission Strength
    ...
    bkit_wireframe.links.new(invert_color.outputs[0], principled_bsdf.inputs[4])
    ...
    bkit_wireframe.links.new(white_noise_texture.outputs[0], hue_saturation_value.inputs[0])
    # mix.Result -> principled_bsdf.Emission Color
    bkit_wireframe.links.new(mix.outputs[2], principled_bsdf.inputs[26])
